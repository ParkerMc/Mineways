...
#include "./village.h"
#include "../labelledslidertwo.h" // Added
...
Minutor::Minutor(): maxentitydistance(0) {
  ...
  if (!settings->verticalDepth) {
    ...
    lowest = new LabelledSliderTwo(Qt::Horizontal); // Added
    mainLayout->addWidget(lowest); // Added
    mainLayout->addWidget(mapview, 1);
  } else {
    mainLayout = new QHBoxLayout;
    depth = new LabelledSlider(Qt::Vertical);
    lowest = new LabelledSliderTwo(Qt::Vertical); // Added
    mainLayout->addWidget(mapview, 1);
    mainLayout->addWidget(lowest); // Added
    mainLayout->addWidget(depth);
  ...
  //TO-DO add update system
  updatesAct->setEnabled(false);
}
...
void Minutor::createActions() {
  ...
  // Added
  // [File]
  terrainAct = new QAction(tr("Set Terrain File"), this);
  terrainAct->setShortcut(tr("Ctrl+T"));
  terrainAct->setStatusTip(tr("Set Terrain File"));
  //connect(terrainAct, SIGNAL(triggered()),
          //this,    SLOT(open()));

  importSettingsAct = new QAction(tr("Import Settings"), this);
  importSettingsAct->setShortcut(tr("Ctrl+I"));
  importSettingsAct->setStatusTip(tr("Import Settings"));
  //connect(importSettingsAct, SIGNAL(triggered()),
  //        this,    SLOT(open()));

  expRenderAct = new QAction(tr("Export for Rendering"), this);
  expRenderAct->setShortcut(tr("Ctrl+R"));
  expRenderAct->setStatusTip(tr("Export for Rendering"));
  //connect(expRenderAct, SIGNAL(triggered()),
  //        this,    SLOT(open()));

  exp3DprintAct = new QAction(tr("Export for 3D Printing"), this);
  exp3DprintAct->setShortcut(tr("Ctrl+P"));
  exp3DprintAct->setStatusTip(tr("Export for 3D Printing"));
  //connect(exp3DprintAct, SIGNAL(triggered()),
  //        this,    SLOT(open()));

  sketchfabAct = new QAction(tr("Publish to Sketchfab"), this);
  sketchfabAct->setShortcut(tr("Ctrl+F"));
  sketchfabAct->setStatusTip(tr("Publish to Sketchfab"));
  //connect(sketchfabAct, SIGNAL(triggered()),
  //        this,    SLOT(open()));

  expSchematicAct = new QAction(tr("Export Schematic"), this);
  expSchematicAct->setShortcut(tr("Ctrl+E"));
  expSchematicAct->setStatusTip(tr("Export Schematic"));
  //connect(expSchematicAct, SIGNAL(triggered()),
  //        this,    SLOT(open()));

  repeatAct = new QAction(tr("Repeat Export"), this);
  repeatAct->setShortcut(tr("Ctrl+X"));
  repeatAct->setStatusTip(tr("Repeat Export"));
  //connect(repeatAct, SIGNAL(triggered()),
  //        this,    SLOT(open()));
  // End added
}
...
void Minutor::createMenus() {
  ...
  // Added
  fileMenu->addSeparator();
  fileMenu->addAction(terrainAct);
  fileMenu->addAction(importSettingsAct);
  fileMenu->addSeparator();
  fileMenu->addAction(expRenderAct);
  fileMenu->addAction(exp3DprintAct);
  fileMenu->addAction(sketchfabAct);
  fileMenu->addAction(expSchematicAct);
  fileMenu->addAction(repeatAct);
  // End added
  fileMenu->addSeparator();
